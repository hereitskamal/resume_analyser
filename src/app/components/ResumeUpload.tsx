// components/ResumeUpload.tsx (or update AdvancedUpload.tsx)
'use client'
...
// Only import pdfjs on client side
...
export default function ResumeUpload({ onAnalysisComplete, onAnalysisStart }: ResumeUploadProps) {
    ...
    const initializePdfJs = async () => {
      if (typeof window !== 'undefined' && !pdfjs) {
        try {
          ...
          setError('PDF processing not available')
        }
      }
    }
  ...
  // PDF text extraction with browser check
  ...
  // DOCX text extraction with browser check
  const extractTextFromDOCX = async (file: File): Promise<string> => {
    if (typeof window === 'undefined') {
      throw new Error('DOCX processing is only available in the browser')
    }
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      if (file.type === 'application/pdf') {
        if (!pdfLoaded) {
          throw new Error('PDF processing is still loading. Please wait a moment and try again.')
        }
